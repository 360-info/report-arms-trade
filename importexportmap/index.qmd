    ...
    function updatePopup(info, event) {
      if (info && info.object) {
          ...
          popup
            ...
            // set coordinates with event.center?
            .setHTML(
              `${info.object.supplier} → ${info.object.recipient}: ${info.object.count > 0 ? info.object.count : 0.5} million`)
            .addTo(map);
        } else {
          popup.remove();
        }
