...
filterSummary = filteredTransferCount > 0 ?
  md`**${filteredTransferCount}** transfers selected totalling at least **$${filteredTransferSum > 1000 ? (filteredTransferSum / 1000) + " billion" : filteredTransferSum + " million" }** TIV` :
  md`No transfers selected`;
...
transferArcs = new deck.MapboxLayer({
  ...
  getTilt: d => Number(d.supplier_lon) > Number(d.recipient_lon) ? 15 : -15,
  pickable: true,
  autoHighlight: true,
      ...
      if (info && info.object) {
        ...
        popup
          ...
          .setHTML(
            ...
            TIV ${info.object.count > 0 ? "$" + info.object.count : "< $0.5"} M in ${info.object.year}</br>
...
filteredTransferCount =
  filteredTransfers.map(d => d.count).length;
filteredTransferSum = {
  const tivsToSum = filteredTransfers.map(d => d.count);
